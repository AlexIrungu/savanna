...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const checkInFormatted = formData.checkInDate ? new Date(formData.checkInDate).toLocaleDateString('en-US', { 
        weekday: 'long', 
        year: 'numeric', 
        month: 'long', 
        day: 'numeric' 
      ...
      const checkOutFormatted = formData.checkOutDate ? new Date(formData.checkOutDate).toLocaleDateString('en-US', { 
        weekday: 'long', 
        year: 'numeric', 
        month: 'long', 
        day: 'numeric' 
      ...
      const templateParams = {
        ...
        dates_of_stay: `${checkInFormatted} to ${checkOutFormatted} (${nights} nights)`,
  ...
  const handleChange = (e) => {
    ...
    // If changing check-in date and it's after check-out date, clear check-out date
  ...
  return (
    <section className="py-32 bg-gradient-to-br from-ghost-white via-vanilla/20 to-savanna-cream/10">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-raleway text-eerie-black/70 mb-1">
                          Check-in Date
                        </label>
                        <input
                          type="date"
                    ...
                    {formData.checkInDate && formData.checkOutDate && (
                      <div className="mt-2 text-sm font-raleway text-beaver">
                        Duration: {Math.ceil((new Date(formData.checkOutDate) - new Date(formData.checkInDate)) / (1000 * 60 * 60 * 24))} nights
                      </div>
                    )}
                  </div>
